...
def get_links(VIM, gene_names, regulators, sort=True, file_name=None):
    ...
    pred_edges = [(gene_names[j], gene_names[i], score) for (i, j),score in np.ndenumerate(VIM) if j in idx] # and i!=j]
...
def xgbgrn(TS_data, time_points, alpha="from_data",  SS_data=None, gene_names=None,regulators='all', param={}):
